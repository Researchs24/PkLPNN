...
% -\eta \right.$(37b)                         $x_i =T_{\lambda } \left(u_i \right)=\left\lbrace 
% \begin{array}{cc}0 & \left|u_i \right|\le \lambda \\u_i -\lambda \textrm{sign}\left(u_i 
% \right) & \left|u_i \right|>\lambda \end{array}\right.$(35)
% This function is used by the ODE solver to compute the solution
% System dynamics
%                   dx/dt = phi'*(r - phi*x) - lambda*grad_x||x||_{k,1}
%                   d
%f = @(sx) 1./(1+exp(-sx)) - 1./(1+exp(sx)); % The function expression
%   TODO: Add the theoritical logic behind these expressions
